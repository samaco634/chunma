            ...
            while True:
                ...
                if ev_lines:
                    for line in ev_lines:
                        ...
                        self.transitionOccurred(event)   
        except KeyboardInterrupt:
             chip.close()
            sys.exit(130)

    def transitionOccurred(self, event):
        p1 = self.lines[0].get_value()
        p2 = self.lines[1].get_value()
        if event.source.offset() == ??? : # p1 이면
            if event.type == gpiod.LineEvent.RISING_EDGE:
                newState = "{}{}".format(1, self.state[1])
            elif event.type == gpiod.LineEvent.FALLING_EDGE:
                newState = "{}{}".format(0, self.state[1])
            else : #invalid state
                newState = self.state
        elif event.source.offset() == ??? : #p2 이면
            if event.type == gpiod.LineEvent.RISING_EDGE:
                newState = "{}{}".format(self.state[0], 1)
            elif event.type == gpiod.LineEvent.FALLING_EDGE:
                newState = "{}{}".format(self.state[0], 0)
            else : #invalid state
                newState = self.state
