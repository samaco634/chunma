...
class EncoderEvdev:
  def __init__(self, callback=None):
    self.d = evdev.InputDevice('/dev/input/by-path/platform-rotary@11-event')
    print('Rotary encoder device: {}'.format(self.d.name))
    ...
    while 1:
      for e in self.d.read_loop():
          ...
          if e.type == evdev.ecodes.EV_REL:
              ...
              if(e.value > 0):
                self.callback(self.position, "R")
              elif(e.value < 0):
                self.callback(self.position, "L")
    self.d.ungrab()
